...
- 分享链接支持访问密码。
- 分享链接支持最大访问次数。
- 分享状态返回访问次数、最后访问时间、密码保护状态、过期状态和访问上限状态。
- 公开访问会先做权限校验和统计记录，再返回缓存后的公开 DTO。
- MockMvc 已覆盖密码访问、过期链接、访问次数限制、分享设置更新和密码清除。

后续优化：

1. 访问量较高后，把浏览统计拆到 Redis 计数或异步批量落库。
2. 增加访问来源、User-Agent、地域等更细粒度统计。
3. 增加二维码分享和一键复制分享文案。
